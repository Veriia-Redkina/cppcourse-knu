...
int main() {
   ...
   int resultA = 0;
   int resultB = 0;
//   for (auto& item : collection) {  //   Замість копіювання використовуємо посилання: додаємо &
   for (auto const & item : collection) { // Змінна item у циклі не змінюється (тільки читається), тому можна використовувати const
